...
function PortugueseSubject({s,setS,go,view="home"}){
  ...
  if(session.finished){
    ...
    return <Shell><p className="eyebrow">{session.label}</p><h1>Sessão concluída</h1><div className="portugueseResultHero"><b>{correct}/{deterministic.length}</b><span>respostas determinísticas corretas</span></div>{missionEvidenceFocus.length>0&&<div className="notice"><b>Esta missão foi ajustada ao teu histórico</b><span>Incluiu critérios que assinalaste anteriormente como “em parte”, “não identificados” ou “por confirmar”. Isto orienta o treino, mas não é uma nota.</span><ul>{missionEvidenceFocus.slice(0,3).map(row=><li key={row.competencyId+row.observationId}>{row.label}</li>)}</ul></div>}
      ...
      <button className="primary" onClick={()=>{setSession(null);go("home")}}>Voltar à Home</button><button className="secondary" onClick={()=>{setSession(null);go("train")}}>Treinar outra coisa</button>
    </Shell>;
  }
  ...
  return <Shell className="studySessionShell"><StudySessionHeader progress={((session.current+1)/session.items.length)*100} label={`${session.current+1}/${session.items.length}`} onExit={pauseSession}/><div className="sessionContext"><small>{session.label}</small><b>{PORTUGUESE_DOMAIN_LABELS[item.domain]} · {item.year}</b></div>
    ...
    {feedback&&<div className={`portugueseFeedback ${feedback.final?(feedback.correct?"correct":"incorrect"):"provisional"}`}><b>{feedback.final?(feedback.correct?"✓ Muito bem!":"Não é essa."):feedback.rubricCompleted?"Autoavaliação guardada — sem classificação automática":"Agora revê a tua resposta"}</b>
