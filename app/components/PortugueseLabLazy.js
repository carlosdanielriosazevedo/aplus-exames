...
function PortugueseLab({s,setS,go}){
  ...
  if(!session)return <Shell><Back go={go}/><div className="portugueseLabHead"><span>Aa</span><div><p className="eyebrow">PILOTO CONTROLADO</p><h1>Português · Prova 639</h1></div></div>
    ...
    <div className="notice"><b>300 itens disponíveis · publicação geral ainda bloqueada</b><span>A base já tem dimensão suficiente para o piloto controlado. O que falta fechar é a calibração de dificuldade, a consistência editorial entre competências e a validação da correção aberta com respostas reais de alunos.</span></div>
