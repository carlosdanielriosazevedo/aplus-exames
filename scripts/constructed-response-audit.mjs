...
assert.equal(CONSTRUCTED_RESPONSE_BANK.length,31);
...
assert.equal(new Set(CONSTRUCTED_RESPONSE_BANK.map(q=>`${q.themeId}:${q.microcompetencyId}`)).size,31,"Cada pergunta construída deve alargar a cobertura a uma competência distinta.");
assert.deepEqual(
