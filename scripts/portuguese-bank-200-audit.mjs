...
assert.equal(packs.at(-1)?.wave,10);
assert.equal(packs.at(-1)?.bankSizeAfterWave,200);
assert.equal(items.length,200,"O banco foundation deve conter exatamente 200 itens.");
assert.equal(new Set(items.map(item=>item.id)).size,200,"IDs duplicados no banco de Português.");
assert.equal(new Set(items.map(item=>item.stimulus)).size,200,"Existem estímulos repetidos.");
...
for(let wave=1;wave<=10;wave++) assert.match(runtime,new RegExp(`portuguese-639-wave${wave}\\.json`),`wave${wave} não está ligada ao runtime.`);
assert.equal((runtime.match(/portuguese-639-(?:pilot|wave\d+)\.json/g)||[]).length,11,"Runtime deve carregar os 11 pacotes foundation.");

console.log(`✓ Portuguese 639: ${items.length} itens · 16 competências escritas cobertas · waves 1-10 ligadas ao runtime · original-only · sem IDs/estímulos duplicados`);
