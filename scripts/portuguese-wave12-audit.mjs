...
const pack=JSON.parse(readFileSync(new URL("../content/vnext/portuguese/foundation/portuguese-639-wave12.json",import.meta.url),"utf8"));
assert.equal(pack.wave,12);
assert.equal(pack.bankSizeAfterWave,250);
...
for(const item of pack.items){
  assert.match(item.id,/^PT639-FND-2(3|4)\d$/u);
...
assert.ok(Object.values(years).every(n=>n>=7));
assert.ok(Object.values(domains).every(n=>n>=5));
assert.ok(pack.items.filter(i=>i.responseType!=="multiple-choice").length>=8);
console.log("✓ Portuguese wave 12: 25 itens · "+JSON.stringify(years)+" · "+JSON.stringify(domains));
