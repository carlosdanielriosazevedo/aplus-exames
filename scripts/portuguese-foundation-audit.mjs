...
assert.equal(packFiles.length,11,"o runtime atual de Português deve ter piloto + dez vagas");
...
assert.equal(latestDeclared,200,"a décima vaga deve elevar o banco para 200 itens");
...
assert.equal(packs.at(-1).wave,10); assert.equal(packs.at(-1).bankSizeAfterWave,200);

assert.equal(new Set(allItems.map(item=>item.id)).size,200,"Os IDs devem ser únicos.");
assert.equal(new Set(allItems.map(item=>item.stimulus)).size,200,"Os estímulos devem ser distintos.");
...
for(const [domain,count] of Object.entries(domainCounts))assert.ok(count>=45,`${domain}: desequilíbrio excessivo (${count}/200).`);
...
assert.equal((runtimeContent.match(/portuguese-639-(?:pilot|wave\d+)\.json/g)||[]).length,11,"O runtime deve agregar piloto + dez vagas."); assert.match(runtimeContent,/flatMap\(pack=>pack\.items\)/);
console.log(`✓ Portuguese 639 foundation: 11 pacotes · 200 itens · 16 competências escritas · distribuição por domínio ${JSON.stringify(domainCounts)} · release ainda bloqueado`);
