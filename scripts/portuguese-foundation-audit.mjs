...
assert.equal(packs.at(-1).wave,12); assert.equal(packs.at(-1).bankSizeAfterWave,250);

assert.equal(new Set(allItems.map(item=>item.id)).size,250,"Os IDs devem ser únicos.");
assert.equal(new Set(allItems.map(item=>item.stimulus)).size,250,"Os estímulos devem ser distintos.");
...
for(const [domain,count] of Object.entries(domainCounts))assert.ok(count>=55,`${domain}: desequilíbrio excessivo (${count}/250).`);
...
console.log(`✓ Portuguese 639 foundation: 13 pacotes · 250 itens · 16 competências escritas · distribuição por domínio ${JSON.stringify(domainCounts)} · release ainda bloqueado`);
