...
assert.equal(packs.at(-1).wave,15); assert.equal(packs.at(-1).bankSizeAfterWave,310);

assert.equal(new Set(allItems.map(item=>item.id)).size,310,"Os IDs devem ser únicos.");
assert.equal(new Set(allItems.map(item=>item.stimulus)).size,310,"Os estímulos devem ser distintos.");
...
for(const [domain,count] of Object.entries(domainCounts))assert.ok(count>=40,`${domain}: desequilíbrio excessivo (${count}/310).`);
...
assert.match(page,/Matemática A e Português disponíveis/); assert.match(page,/preview==="portuguese"/); assert.match(portugueseSubject,/function PortugueseSubject\(/);
