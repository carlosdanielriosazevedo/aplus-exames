...
import {PORTUGUESE_COMPETENCIES,PORTUGUESE_CURRICULUM_GOVERNANCE,PORTUGUESE_DOMAINS,PORTUGUESE_REFERENCE_SOURCES,PORTUGUESE_RELEASE_POLICY,PORTUGUESE_RESPONSE_TYPES,PORTUGUESE_YEAR_FOCUS} from "../app/data/portugueseFoundation.js";
...
assert.equal(packFiles.length,11,"o runtime atual de Português deve ter piloto + onze vagas");
...
assert.equal(latestDeclared,225,"a décima vaga deve elevar o banco para 200 itens");
...
assert.ok(PORTUGUESE_RELEASE_POLICY.minimumBetaItems>allItems.length,"200 itens ainda não devem desbloquear automaticamente o beta de Português");
...
for(const competency of PORTUGUESE_COMPETENCIES.filter(c=>c.writtenExam))assert.ok(allItems.filter(item=>item.competencyId===competency.id).length>=7,`${competency.id}: menos de sete itens.`);
...
for(const [domain,count] of Object.entries(domainCounts))assert.ok(count>=50,`${domain}: desequilíbrio excessivo (${count}/225).`);
...
assert.equal((runtimeContent.match(/portuguese-639-(?:pilot|wave\d+)\.json/g)||[]).length,11,"O runtime deve agregar piloto + onze vagas."); assert.match(runtimeContent,/flatMap\(pack=>pack\.items\)/);
console.log(`✓ Portuguese 639 foundation: 12 pacotes · 225 itens · 16 competências escritas · distribuição por domínio ${JSON.stringify(domainCounts)} · release ainda bloqueado`);
