...
assert.equal(latestDeclared,275,"a décima vaga deve elevar o banco para 200 itens");
...
assert.equal(packs.at(-1).wave,13); assert.equal(packs.at(-1).bankSizeAfterWave,275);

assert.equal(new Set(allItems.map(item=>item.id)).size,275,"Os IDs devem ser únicos.");
assert.equal(new Set(allItems.map(item=>item.stimulus)).size,275,"Os estímulos devem ser distintos.");
...
for(const [domain,count] of Object.entries(domainCounts))assert.ok(count>=40,`${domain}: desequilíbrio excessivo (${count}/275).`);
...
console.log(`✓ Portuguese 639 foundation: 14 pacotes · 275 itens · 16 competências escritas · distribuição por domínio ${JSON.stringify(domainCounts)} · release ainda bloqueado`);
