...
const targets=[
  ["app/data/vnextPilot.js","VNEXT_PILOT_QUESTIONS",VNEXT_PILOT_QUESTIONS,`export const VNEXT_PILOT_META=${JSON.stringify(VNEXT_PILOT_META,null,2)};`],
...
let changedItems=0;
for(const [relative,name,items,metaExport] of targets){
  ...
  fs.writeFileSync(path.resolve(relative),`${header}${metaExport?`${metaExport}\n\n`:""}export const ${name}=${JSON.stringify(polished,null,2)};\n`);
}
