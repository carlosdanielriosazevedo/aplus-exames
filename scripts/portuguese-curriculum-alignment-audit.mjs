...
for(const year of YEARS){
  ...
  assert.ok(focus.sourceId,`${year}: falta fonte DGE.`);
  ...
  assert.ok(items.length>=50,`${year}: menos de 50 itens no banco atual.`);
  const domains=Object.fromEntries(WRITTEN_DOMAINS.map(domain=>[domain,items.filter(item=>item.domain===domain).length]));
  for(const [domain,count] of Object.entries(domains))assert.ok(count>=10,`${year}: cobertura demasiado baixa em ${domain} (${count}).`);
  ...
  report[year]={items:items.length,domains,competencies:competencies.size};
}
...
for(const item of open){
  ...
  for(const criterion of item.rubric.criteria){
    assert.ok(criterion.observations?.length,`${item.id}/${criterion.id}: falta decomposição observável.`);
  }
}
...
console.log("✓ Alinhamento curricular e de correção verificado estruturalmente.");
