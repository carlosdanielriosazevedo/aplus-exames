...
const wave=JSON.parse(readFileSync(new URL("../content/vnext/portuguese/foundation/portuguese-639-wave7.json",import.meta.url),"utf8"));
const items=wave.items;
const written=new Map(PORTUGUESE_COMPETENCIES.filter(row=>row.writtenExam).map(row=>[row.id,row]));

assert.equal(wave.wave,7);
assert.equal(wave.bankSizeAfterWave,140);
...
assert.deepEqual(items.map(item=>item.id),Array.from({length:20},(_,i)=>`PT639-FND-${String(121+i).padStart(3,"0")}`));
...
assert.deepEqual(byDomain,{leitura:5,"educacao-literaria":5,escrita:3,gramatica:7});
const open=items.filter(item=>["restricted-response","extended-writing"].includes(item.responseType));
assert.equal(open.length,5,"wave 7 deve reforçar respostas abertas com cinco itens");
assert.ok(open.some(item=>item.responseType==="extended-writing"),"wave 7 deve incluir produção escrita extensa");
assert.ok(open.every(item=>item.gradingMode==="rubric-assisted"),"respostas abertas devem manter correção assistida");
assert.ok(items.filter(item=>item.cognitive==="criar").length>=3,"wave 7 deve reforçar operação criar");
...
assert.ok(positions.every(n=>n>=1),`todas as posições A/B/C/D devem ocorrer: ${positions.join("/")}`);
...
console.log(`✓ wave 7 Português: 20 itens · anos ${JSON.stringify(byYear)} · domínios ${JSON.stringify(byDomain)} · ${open.length} respostas abertas · posições MC ${positions.join("/")} · banco 140`);
